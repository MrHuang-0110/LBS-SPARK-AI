...
- 支持 `0xC2`/`0xC0` 主动进入/退出遥控模式
- 支持 `0xC1` 蓝牙遥控数据

### 📊 实时监控上报（JSON）

通过 USB CDC（每轮主循环）和蓝牙 BLE（约每 200ms）输出 JSON 格式的设备状态，字段包括：
...
| 1 | sID | 源 ID (`0x97`) |
| 2 | oID | 目标 ID (`0x98`) |
| 3 | length | payload 长度（1 字节） |
| 4 | index | 指令类型 |
| 5–(4+length) | data | 数据载荷 |
| 5+length | crc | 校验和 |
| 6+length | tard | `0xA5` 帧尾 |

整帧长度为 `length + 7` 字节；`length` 可以为 `0`。

### 指令索引

| Index | 功能 | 说明 |
| ------- | ------ | ------ |
| `0xB6` | 运行脚本 | 运行当前选中脚本，幂等，重复下发不会重启；USB/蓝牙语义一致 |
| `0xB9` | 停止脚本 | 幂等停止当前脚本或 Pauto；USB/蓝牙语义一致 |
...
| `0xC2` | 进入遥控模式 | 主动运行 `remote.o`；若蓝牙未打开则打开并写入 `blue_cfg.cfg` |
| `0xC0` | 退出遥控模式 | 仅退出当前遥控模式并清空遥控数据；不自动关闭蓝牙，不误停普通脚本 |
| `0xC1` | 蓝牙遥控 | 遥控器数据 |
| `0xC3` | 查询存储使用 | 保留 |
| `0xC4` | 查询文件列表 | 保留 |

> 命令帧由 `ble_receive`（1ms 事件）与 `usb_receive`（1ms 事件）分别派发到同一个 `busDataparsing()`，主循环被脚本阻塞时命令仍可执行；OTA 数据帧统一进入主循环 `touchOtherFile()` 写入 Flash。零长度进入/退出遥控帧示例：`5A 97 98 00 C2 4B A5` / `5A 97 98 00 C0 49 A5`。
...
4. `postbuild.bat` 自动将 `.bin` 复制到 `E:\LBS-FramWare\products\SPARK-AI\fwlib\app\app.bin`

> ⚠️ **注意**：`postbuild.bat` 中的源路径和目标路径均为硬编码绝对路径（当前源目录为 `E:\LBS-SPARK-AI`）。如果仓库或固件工具目录变更，需同步修正，否则下载工具可能烧录旧版本固件。
