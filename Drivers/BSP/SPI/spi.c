...
{
    HAL_SPI_Transmit(&g_spi2_handler,data,length,1000);
}
void spi2_receive_more_byte(uint8_t *data,uint16_t lengt)
{
    HAL_SPI_Receive(&g_spi2_handler,data,lengt,1000);
}
void spi2_tx_rx_more_byte(uint8_t *txdata,uint8_t *rxdata,uint16_t length)
{  
    HAL_SPI_TransmitReceive(&g_spi2_handler, txdata, rxdata, length, 1000);
}
